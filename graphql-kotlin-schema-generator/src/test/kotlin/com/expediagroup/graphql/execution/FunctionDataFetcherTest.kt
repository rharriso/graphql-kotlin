...
internal class FunctionDataFetcherTest {
    ...
    internal class MyClass {
        ...
        fun resultList(items: List<String>): Result<String, GraphQLException> =
            Ok(items.joinToString(separator = ":"))

        suspend fun suspendResultList(items: List<String>): Result<String, GraphQLException> = coroutineScope {
            Ok(items.joinToString(separator = ":"))
        }

        fun resultFailure(): Result<String, GraphQLException> =
            Err(GraphQLException("Failed to produce result"))

        suspend fun suspendResultFailure(): Result<String, GraphQLException> = coroutineScope {
            Err(GraphQLException("Failed to produce result"))
        }
    }
    ...
    fun `null target and null source returns null`() {
        ...
        every { mockEnvironment.getSource<Any>() } returns null
    ...
    fun `null target and valid source returns the value`() {
        val dataFetcher = FunctionDataFetcher(target = null, fn = MyClass::print)
        val mockEnvironment: DataFetchingEnvironment = mockk()
        every { mockEnvironment.getSource<Any>() } returns MyClass()
        every { mockEnvironment.arguments } returns mapOf("string" to "hello")
    ...
    fun `valid target with context class`() {
        ...
        val mockEnvironment: DataFetchingEnvironment = mockk()
        every { mockEnvironment.getContext<MyContext>() } returns MyContext("foo")
    ...
    fun `array inputs can be converted by the object mapper`() {
        ...
        val mockEnvironment: DataFetchingEnvironment = mockk()
        every { mockEnvironment.arguments } returns mapOf("items" to arrayOf("foo", "bar"))
    ...
    fun `list can be converted by the object mapper`() {
        ...
        val mockEnvironment: DataFetchingEnvironment = mockk()
        every { mockEnvironment.arguments } returns mapOf("items" to listOf("foo", "bar"))
    ...
    fun `dataFetchingEnvironement is passed as an argument`() {
        ...
        val mockEnvironment: DataFetchingEnvironment = mockk()
        every { mockEnvironment.field } returns mockk {
    ...
    fun `renamed fields can be converted by the object mapper`() {
        ...
        every { mockEnvironment.arguments } returns arguments
    ...
    fun `resultList returns a successful result when resolved`() {
        ...
        val mockEnvironment: DataFetchingEnvironment = mockk()
        every { mockEnvironment.arguments } returns mapOf("items" to listOf("foo", "bar"))
        val result = dataFetcher.get(mockEnvironment) as Result<*, *>

        result.onFailure { throw Exception("Should not be here") }
            .onSuccess { assertEquals("foo:bar", result.get()) }
    }
    ...
    fun `resultFailure returns a failing result when resolved`() {
        ...
        val result = dataFetcher.get(mockEnvironment) as Result<String, GraphQLException>
        result.onSuccess { throw Exception("Should not be here") }
            .onFailure { assertEquals("Failed to produce result", result.getError()?.message) }
    }
    ...
    fun `suspendResultList returns a successful result when resolved`() {
        ...
        val mockEnvironment: DataFetchingEnvironment = mockk()
        every { mockEnvironment.arguments } returns mapOf("items" to listOf("foo", "bar"))
        val resultFuture = runBlocking { dataFetcher.get(mockEnvironment) } as CompletableFuture<Result<String, GraphQLException>>
